...
public class StoredData implements Serializable {
    ...
    // Calculate CO2 for daily emissions
    public double getCO2ForDay(String date) {
        Map<String, List<List<String>>> activityStrings = new HashMap<>();
        if (this.s_data.containsKey(date)) {
            activityStrings.put(date, this.s_data.get(date));
        }
        StoredData dailyData = new StoredData(activityStrings);
        double CO2 = 0;
        for (AbstractActivities a : dailyData.activities) {
            CO2 += a.calculateCO2();
        }
        return CO2;
    }

    // Calculate CO2 for weekly emissions
    public double getCO2ForWeek(String startDate) {
        double totalCO2 = 0;
        for (int i = 0; i < 7; i++) {
            String date = getDateFromOffset(startDate, i); // You can implement this method to get dates for the whole week
            totalCO2 += getCO2ForDay(date);
        }
        return totalCO2;
    }

    // Calculate CO2 for monthly emissions
    public double getCO2ForMonth(int year, int month) {
        double totalCO2 = 0;
        for (int day = 1; day <= getDaysInMonth(year, month); day++) { // You can implement getDaysInMonth() for the number of days
            String date = encode_date(year, month, day);
            totalCO2 += getCO2ForDay(date);
        }
        return totalCO2;
    }

    // Calculate CO2 for yearly emissions
    public double getCO2ForYear(int year) {
        double totalCO2 = 0;
        for (int month = 1; month <= 12; month++) {
            totalCO2 += getCO2ForMonth(year, month);
        }
        return totalCO2;
    }

    // Helper method to get a date offset from a start date
    ...
    // Helper method to get number of days in a month
